...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        property: "og:title",
        content: "Coding Club Event Attendance",
      },
      {
        property: "og:description",
        content:
          "Search registered teams and mark student attendance from your phone.",
      },
...
function AttendancePage() {
  ...
  const {
    ...
    // Automatically reload Google Sheet data every 10 seconds
    refetchInterval: 10000,

    // Continue checking even if the browser tab is in the background
  ...
  const mutation = useMutation({
    ...
    onError: (err, _vars, ctx) => {
      if (ctx?.prev) {
        qc.setQueryData(["students"], ctx.prev);
      }
